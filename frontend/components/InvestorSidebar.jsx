...
export default function InvestorSidebar({ isMobile = false, onNavigate }) {
  ...
  const links = [
    ...
    { name: "Explore Deals", href: "/investor/explore", icon: Compass },
    { name: "Startups", href: "/investor/startups", icon: Briefcase },
    { name: "Saved Pipeline", href: "/investor/startups/saved", icon: Bookmark },
    {
      name: "Inbox",
      href: "/investor/inbox",
      icon: Mail,
  ...
  return (
    ...
    >
      ...
      {!isMobile && (
        <div className="flex items-center justify-between p-4 border-b border-slate-100 h-16">
          <Link
            ...
            className={`font-black text-xl text-emerald-600 flex items-center gap-2 truncate transition-all duration-300 ${
              effectiveExpanded ? "opacity-100 w-auto" : "opacity-0 w-0 overflow-hidden"
            }`}
          >
            <span className="w-8 h-8 rounded-xl bg-emerald-600 text-white flex items-center justify-center font-black text-base shadow-sm">
              F
            </span>
            <span>Foundry</span>
          </Link>
      ...
      >
        {links.map((link) => {
          const isActive =
            pathname === link.href || (pathname.startsWith(`${link.href}/`) && link.href !== "/investor/startups");
